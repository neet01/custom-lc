...
export const slackArchiveSchema: ExtendedJsonSchema = {
  ...
  properties: {
    ...
    topic: {
      type: 'string',
      description:
        'For advanced_search_messages: the core topic or subject to search with the indexed Slack enterprise-memory backend. Pass the distilled subject, not the full user question.',
    },
    conversationId: {
      type: 'string',
      description:
...
export const toolDefinitions: Record<string, ToolRegistryDefinition> = {
  ...
  spreadsheet_transform: {
    ...
    description:
      'Inspect attached spreadsheets and create downloadable spreadsheet exports directly in chat. Use this to inspect workbook structure, clean columns, add or update rows and cells, calculate values, sort data, or merge and split sheets.',
  ...
  slack_archive_search: {
    ...
    description:
      'Search and retrieve archived GovSlack messages. Prefer advanced_search_messages for indexed semantic/hybrid discovery using EnterpriseMemory text-index chunks; use get_messages for exact per-conversation context and search_messages only as lexical fallback.',
